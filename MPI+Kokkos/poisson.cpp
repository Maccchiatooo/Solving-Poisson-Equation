...
KOKKOS_FUNCTION double u_exact(double x, double y)
{
    double pi = 3.141592653589793;
    double value = sin(pi * x * y);
    return value;
}
KOKKOS_FUNCTION double uxxyy_exact(double x, double y)
{
    double pi = 3.141592653589793;
    double value;
...
void Poisson2d::init()
{
    ...
    m_cellValue = Kokkos::View<double **, Kokkos::CudaSpace>("m_cellValue", lx + 2, ly + 2);
    ...
    Kokkos::parallel_for(
        "init", mdrange_policy({0, 0}, {X - 1, Y - 1}), KOKKOS_CLASS_LAMBDA(const int i, const int j) {
            double phi_x = (lx * comm.px + i - 1) * dx;
            double phi_y = (ly * comm.py + j - 1) * dy;

            if ((lx * comm.px + i - 1 == 0) || (lx * comm.px + i - 1 == glx - 1) || (ly * comm.py + j - 1 == 0) || (ly * comm.py + j - 1 == gly - 1))
            {
...
void Poisson2d::setup_subdomain()
{
    ...
    if (x_lo != 0)
        m_leftout = buffer_t("m_left", y_hi - y_lo + 2);

    if (x_hi != glx)
        m_rightout = buffer_t("m_right", y_hi - y_lo + 2);

    if (y_lo != 0)
        m_downout = buffer_t("m_down", x_hi - x_lo + 2);

    if (y_hi != gly)
        m_upout = buffer_t("m_up", x_hi - x_lo + 2);
}
void Poisson2d::pack()
{

    m_leftout = Kokkos::subview(m_cellValue, 1, Kokkos::ALL);
    m_rightout = Kokkos::subview(m_cellValue, lx + 1, Kokkos::ALL);
    m_downout = Kokkos::subview(m_cellValue, Kokkos::ALL, 1);
    m_upout = Kokkos::subview(m_cellValue, Kokkos::ALL, ly + 1);

    m_left = Kokkos::subview(m_cellValue, 0, Kokkos::ALL);
    m_right = Kokkos::subview(m_cellValue, lx + 2, Kokkos::ALL);
    m_down = Kokkos::subview(m_cellValue, Kokkos::ALL, 0);
    m_up = Kokkos::subview(m_cellValue, Kokkos::ALL, ly + 2);
}
